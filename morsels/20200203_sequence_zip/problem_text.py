...
>>> zipped[1]
('B', 'easy', 2)
Your SequenceZip objects is only expected to work with sequences (like lists and strings). Your SequenceZip objects shouldn't copy the sequences given to them.
...
For the second bonus, I'd like your SequenceZip class to support equality, meaning you can ask two different SequenceZip objects where they're contain the same items:

>>> SequenceZip('hiya', [1, 2]) == SequenceZip('hi', [1, 2, 3])
True
>>> SequenceZip('hiya', [1, 2, 3]) == SequenceZip('hi', [1, 2, 3])
False
Equality shouldn't loop over every item in all sequences but should delegate equality to the sequences themselves. This can improve performance with some objects (like ranges).
