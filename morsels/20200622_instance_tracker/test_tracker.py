...
from tracker import instance_tracker
...
class InstanceTrackerTests(unittest.TestCase):
    ...
    # @unittest.expectedFailure
    def test_inheritance_tracks_all_instances_custom_name_gcooke(self):
        class Tracked(instance_tracker("_registry")):
            def __init__(self, something):
                self.something = something
                super().__init__()
        class A(Tracked):
        ...
        class B(Tracked):
            def __init__(self, something, x):
                ...
                super().__init__(something)
        a = A(something="a")
        self.assertEqual(set(Tracked._registry), {a})
        b = B("b", 3)
        c = B("c", x=4)
        self.assertEqual(set(Tracked._registry), {a, b, c})
        self.assertEqual(a._registry, b._registry)
        self.assertEqual(b.x, 3)
        self.assertEqual(c.x, 4)
        self.assertEqual(a.something, "a")
        self.assertEqual(c.something, "c")
