...
False
This SequenceZip class is only expected to work with sequences (like lists and strings). SequenceZip objects shouldn't copy the sequences given to them.

You can solve the base problem by copy-pasting a solution to bonus 2 from the original SequenceZip problem. I recommend attempting to re-solve this without relying on your code from before first.

The bonuses might feel a bit contrived this time, but I hope you'll have fun with the challenge of adding new features to this class.
...
For the second bonus, I'd like your SequenceZip class to be mutable. SequenceZip objects should support assignment to indexes and deleting of items based on their index.
...
When assigning into a SequenceZip object, a sequence of the correct length (the number of sequences being wrapped around) must be provided.

Note that mutating SequenceZip objects will fail if they wrap around immutable sequences.
...
For the third bonus I'd like your SequencZip objects to support append and insert operations as well.
