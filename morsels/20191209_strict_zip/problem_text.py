This week you have to write a strict_zip function, which acts like the built-in zip function except that looping over sequences with different lengths should raise an exception.

Your strict_zip function should accept zero or more sequences and should return an iterable of tuples, just like the zip function does.
...
>>> list(strict_zip())
[]
Unlike zip, when strict_zip is passed a sequence which have different numbers of elements, a ValueError should be raised.
...
In the first bonus, strict_zip should take accept all iterables, not just sequences. For this bonus, only finite iterables will be passed to strict_zip.
...
For the third bonus, strict_zip should work with infinitely long iterables, such as itertools.count and itertools.repeat.
